...
if ( !function_exists( 'pendrell_icon' ) ) : function pendrell_icon( $icon = '', $title = '', $desc = '' ) {
  ...
  if ( PENDRELL_ICONS_URL )
    $url = PENDRELL_ICONS_URL;

  // Put it all together
  $output = '<svg class="icon" role="img"' . $aria . '>' . $title . $desc . '<use xlink:href="' . $url . '#i-' . $icon . '" /></use></svg>';
...
if ( PENDRELL_ICONS_PATH && PENDRELL_ICONS_URL === false )
