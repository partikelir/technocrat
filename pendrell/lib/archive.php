...
function pendrell_archive_title() {
  if ( is_day() ) {
    ...
    $title = sprintf( __( '%s archives', 'pendrell' ), pendrell_name_wrapper( single_cat_title( '', false ) ) );
  } elseif ( is_post_type_archive() ) {
    $title = sprintf( __( '%s archives', 'pendrell' ), pendrell_name_wrapper( post_type_archive_title( '', false ) ) );
  } elseif ( is_tag() ) {
    $title = sprintf( __( '%s archives', 'pendrell' ), pendrell_name_wrapper( single_tag_title( '', false ) ) );
  } elseif ( is_tax() ) {
...
function pendrell_archive_description() {
  ...
  if ( !is_paged() ) {
    ...
    if ( is_category() || is_tag() || is_tax() ) {
      ...
      ?><div class="archive-content"><?php // @TODO: make this conditional; it messes up the visual flow
      ...
      if ( !empty( $description ) ) {
        ?><div class="archive-description"><?php echo do_shortcode( $description ); ?></div><?php
      }
