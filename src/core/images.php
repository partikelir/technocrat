...
function pendrell_image_wrapper( $content = '' ) {
  ...
  // Hooks into existing post_thumbnail_size filter; modified for full-width display by another filter later on
...
add_filter( 'post_thumbnail_size', 'pendrell_image_full_width_resize' ); // Post thumbnails in full-width mode
add_filter( 'ubik_imagery_size', 'pendrell_image_full_width_resize' ); // Post contents generated by image shortcodes in full-width mode
...
function pendrell_image_bounding_width() {
...
function pendrell_image_dimensions( $html, $width, $height ) {
  ...
  if ( $width <= $bounding_width )
    return $html;

  $factor = $bounding_width / $width;
